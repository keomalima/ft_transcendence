...
type PlayersInfo = { numberOfPlayers: number; players?: string[] };

export class GameConnection {
	private ws: WebSocket | null = null;

	connect(gameId: string, userId: string, scoreToWin: string): Promise<PlayersInfo> {
		const httpUrl = new URL(`/ws/game/${gameId}/${userId}/${scoreToWin}`, API_BASE_URL);
		httpUrl.protocol = httpUrl.protocol === 'https:' ? 'wss:' : 'ws:';
		this.ws = new WebSocket(httpUrl.href);

		return new Promise<PlayersInfo>((resolve, reject) => {
			const onOpen = () => {
				resolve({ numberOfPlayers: 1 });
			};
			...
			this.ws!.onmessage = (event) => {
				...
				if (data.type == 'number-of-players') {
					resolve({
						numberOfPlayers: data.numberofplayer ?? 1,
					});
